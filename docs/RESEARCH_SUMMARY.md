...
## What a US reseller actually evaluates (address each on the page)
1. **Recurring revenue** — SaaS margin they set + payment residual they keep.
2. **White-label depth** — logo, domain, login, receipts, admin, apps (not a skin).
3. **Bring-your-own processor** — don't make them give up residuals.
4. **Hardware flexibility** — iPad / Android / PAX A920 & A35, no proprietary lock-in.
5. **Low onboarding/support burden** — ~14-day go-live; vendor backs support.
6. **Competitive differentiation** — something they can pitch their clients to switch.

## Competitive landscape (one-liners)
- **Toast** — restaurant-purpose-built but proprietary hardware lock-in, mandatory Toast
  Payments, partner program = one-time bounties; add-ons stack to $400–500/mo.
- **Square** — free/cheap, generic, modest partner economics.
- **Clover (Fiserv)** — app marketplace + ISO residuals, but locked hardware/processor.
- **TouchBistro** — iPad-only, full-service focus.
...
- "Your customer. Your contract. Your brand." (counters captive anxiety)
- "Bring your processor." (keeps ISO residuals)
- A residual + SaaS **ROI calculator** so the partner self-quantifies.
- A **specific 14-day** go-live number (specifics beat adjectives).
- **AI included** framed against incumbents' $75–$200/mo add-ons.
- Honest **"production-ready, waiting for backend integration"** signal to sophisticated buyers.
- Side-by-side **comparison table** to remove the "but my prospect uses Heartland" objection.

## Landing-page best practices applied
- One clear outcome-led H1; dual CTA (test-drive / partner economics); minimal nav.
- Lead with a real, interactive product experience, not abstract 3D.
- Interactive demo embed (here: a working POS test-drive) — interactive demos measurably lift
  engagement/sign-ups.
- Social proof via industry stats + ROI math (no fabricated logos until real ones exist).
...
## Cautions
- Competitor pricing/programs change — label as industry-reported, current early 2026.
- Heartland Restaurant → rebranded "Genius" (use "Heartland / Genius" in comparisons).
- ROI calculator must stay clearly illustrative; ~0.35% reseller margin is a conservative midpoint.
