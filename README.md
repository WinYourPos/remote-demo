...
| **`spark-system.html`** | Interactive product demo. **Order + Kitchen (KDS) + Reports**, wired together: ring an item with modifiers → fire to the kitchen → close the check → reporting updates live. Mobile + landscape optimised so partners can scan & try from any phone (lands on `try.html` first). Now also a standalone **Store Setup** merchant-boarding surface (from the Stations launcher): board a store, then each step plays on a realistic screen that **auto-fills** — Stripe Connect, menu upload, **per-state** tax registration, beneficial-owner KYC, a $1.00 verify charge, go-live — or run the whole boarding live. | [open ↗](https://winyourpos.github.io/remote-demo/spark-system.html) |
